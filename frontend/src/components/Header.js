...
const pages = [{name: 'СУВР', link: 'suvr'}, {name: 'Test', link: 'test'}];
const settings = ['TEST1', 'TEST2', 'TEST3', 'TEST4'];


function Header(props) {
    const navigate = useNavigate()
    const [activeButton, setActiveButton] = useState(getCurrentPage(pages))


    return (
        <nav
            className="bg-gradient-to-r from-sky-950 to-sky-700 border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800 drop-shadow-md">

            <div
                className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">

                <div className={'flex'}><BiSolidCat color={'#b4bac1'} size={40}/>

                <span
                    className="self-center text-xl text-white ml-2 font-semibold whitespace-nowrap dark:text-white">SERVICE</span>
                    </div>
                ...
                <div className="flex items-center lg:order-2">
                    ...
                    <button data-collapse-toggle="mobile-menu-2" type="button"
                        ...
                        <svg className="hidden w-6 h-6" fill="currentColor"
                             viewBox="0 0 20 20"
                             xmlns="http://www.w3.org/2000/svg">
                            <path fillRule="evenodd"
                            ...
                            pages.map((page, index) => (
                                ...
                                <li onClick={(event) => {
                                    ...
                                    <button
                                        id={index}
                                        className={
                                            index === activeButton ? 'underline decoration-2 underline-offset-8 text-white btnNav ' : 'btnNav'
                                        }
